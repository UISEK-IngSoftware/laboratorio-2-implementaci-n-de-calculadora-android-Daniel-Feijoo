...
class CalculatorViewModel : ViewModel() {
    ...
    private fun enterOperator(op: String) {
        // Si ya hay una operación completa, calcula antes de continuar
        if (number1.isNotBlank() && operator != null && number2.isNotBlank()) {
            performCalculation(auto = true)
        }
        // Luego, actualiza el operador al nuevo
        operator = op
    }
    ...
    private fun performCalculation(auto: Boolean = false) {
        ...
        if (num1 != null && num2 != null && operator != null) {
            ...
            // Si fue cálculo automático (por ejemplo al presionar otro operador)
            if (auto) {
                number1 = resultString
                number2 = ""
                state = state.copy(display = resultString)
            } else {
                // Si fue cálculo final (al presionar "=")
                clearAll()
